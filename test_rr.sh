...
for i in "${test_numbers[@]}"; do
    input_file="$test_dir/test_rr_${i}.proc"
    expected_output="$answers_dir/test_rr_${i}.output"
    actual_output="$my_answers_dir/test_rr_${i}.actual"

    # Run the scheduler and redirect output
    ./sched_rr "$input_file" > "$actual_output"
